---
name: skill-architecture-discovery
description: >-
...
([`skill-technical-design`](../skill-technical-design/SKILL.md) /
[`agt-architecture`](../../agents/agt-architecture.md)). Use discovery when the
...
services — use the existing layered agents and `rule.*.mdc` instead.
...
5. **Do not apply `.cursor/rules/`** without an explicit human gate `APPROVED`.
...
4. `examples/canonical-user/` **or** `.cursor/rules/rule.project-core.mdc` / `rule.business-rules-layers.mdc`
...
- Use existing rule.*.mdc / AGENTS.md / docs/architecture-and-layers.md
```
...
- Existing docs: `README*`, `AGENTS.md`, ADRs, architecture docs, `.cursor/rules/`
...
3. Draft rules under `docs/architecture/rule-drafts/` using
   [`templates/rule-draft.mdc`](templates/rule-draft.mdc).
4. Ask for gate: `APPROVED` | `CHANGES_REQUESTED` | `REJECTED`.
5. **Only if `APPROVED`:** copy/merge drafts into `.cursor/rules/`.

#### Rule writing conventions

- If [`.cursor/rules/meta.cursor-rules.mdc`](../../rules/meta.cursor-rules.mdc)
  exists (this kit): use `rule.<kebab>.mdc` / `meta.<kebab>.mdc`, `mdc:` links,
  DO/DON'T examples.
- Else if the target repo has its own rule convention: follow it.
- Else: `kebab-case.mdc` with Cursor frontmatter (`description`, optional
  `globs`, `alwaysApply`).
...
| **Deprecate** | Never delete silently; mark deprecated + migration path (see [`meta.self-improve.mdc`](../../rules/meta.self-improve.mdc)) |

Avoid creating rules that duplicate existing `rule.*.mdc` content in this kit.
