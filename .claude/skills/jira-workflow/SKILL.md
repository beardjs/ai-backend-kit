---
name: jira-workflow
argument-hint: "[issue-key | JQL | create]"
description: >-
...
- [reference.md](reference.md) — org defaults quick reference
- [agt-jira-workflow](../../agents/ops/agt-jira-workflow.md) — orchestrator agent
- Credentials: `JIRA_INSTANCE_URL`, `JIRA_USER_EMAIL`, `JIRA_API_KEY` from the environment or the service's MCP config (`.mcp.json`; `~/.cursor/mcp.json` when the Cursor kit is also in use)
...
| Read tool | Jira/Atlassian MCP server when configured; REST/curl fallback |
...
Load credentials from the environment or MCP config:
...
tools with ToolSearch when they are deferred). Typical read tools:

| Tool shape | When to use | Example args |
|------|-------------|--------------|
| issue lookup (e.g. `get_issue`) | Single issue by key or id | `{ "issueIdOrKey": "PROJ-1487" }` |
| JQL search (e.g. `jql_search`) | List/search issues | `{ "jql": "parent = PROJ-1487", "maxResults": 50 }` |

If no Jira MCP server is configured, read via REST with the same Basic Auth
credentials (`GET /rest/api/3/issue/<key>`, `GET /rest/api/3/search?jql=…`).
...
Create via REST API v3 with `curl` (do not rely on MCP write tools).
