...
- **Errors:** every handler wraps the service call in `try/catch` and translates with `handleTranslatedError(error, ErrorCatalog, res)` from shared packages. The service throws `IThrowedError` carrying an `EErrorCode`; the controller never builds status/body for errors by hand.
...
- **Authorization:** guard routes with `authorizeByGroup([EUserGroup...])` from shared packages as Express middleware in `initRoutes()` (e.g. `this.router.get('/users', authorizeByGroup([EUserGroup.BACKOFFICE, EUserGroup.ADMIN]), this.getUsers)`).
