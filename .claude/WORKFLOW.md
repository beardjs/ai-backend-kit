...
Complements [README.md](README.md) (kit index) and [AGENTS.md](../AGENTS.md) (short contract).

**Default entry for new features:** invoke **`/orchestrate`**. The main thread classifies intent, builds the minimal pipeline, applies gates, and dispatches specialist subagents — it does not implement, write specs, or commit on its own.
...
Skill map (scaffold vs SDD vs review): [README.md](README.md). Canonical shapes: [`examples/canonical-user/`](../examples/canonical-user/).
...
| `tasks.md` | Architecture + `/spec-driven` | With design | Traceable slices (`TASK-*` → `AC-*` / `TC-*`) |
...
| Agent | Does | Does not |
|-------|------|----------|
| [`/orchestrate`](skills/orchestrate/SKILL.md) (main thread) | Classify, sequence, apply gates, synthesize | Edit `src/` or specs |
| [`agt-product-owner`](agents/sdd/agt-product-owner.md) | Requirements, AC, DoR | Code, schema, library choices |
| [`agt-architecture`](agents/sdd/agt-architecture.md) | Technical `design.md` | Edit `src/`; redefine product rules |
| [`agt-quality-assurance`](agents/sdd/agt-quality-assurance.md) | PLAN / VERIFY (AUTOMATE routes to author) | Write Jest; change production to “pass” a test |
| [`agt-test-author`](agents/sdd/agt-test-author.md) | Create / extend unit & integration tests | Change production; own qa-report |
| [`agt-dev-backend`](agents/sdd/agt-dev-backend.md) | Implement the approved slice | Reinterpret an ambiguous rule |
| [`agt-test-runner`](agents/sdd/agt-test-runner.md) | Stabilize Jest / technical regression | Redefine product ACs |
| [`agt-code-review`](agents/review/agt-code-review.md) | Spec ↔ code review (read-only) | Implement fixes |
| [`agt-security-review`](agents/review/agt-security-review.md) | Adversarial security pass (read-only) | Judge spec conformance — that is code review |
| [`agt-architecture-review`](agents/review/agt-architecture-review.md) | Post-code layer audit | Write design (that is `agt-architecture`) |
| [`agt-code-quality`](agents/review/agt-code-quality.md) | Naming + REST | Replace spec-aware code review |
| [`agt-verifier`](agents/sdd/agt-verifier.md) | Delivery evidence | Soften asserts |
...
1. The orchestrator classifies intent as `feature` (or `specify` / `bugfix`).
2. Optional: `agt-jira-workflow` brings card context.
3. `agt-product-owner` (preloaded with `product-refinement`) writes `requirements.md`.
...
1. `agt-architecture` (preloaded with `technical-design`) produces `design.md`.
2. Traceable tasks in `tasks.md` (via `/spec-driven` / tech).
...
1. `agt-dev-backend` (preloaded with `backend-implementation`) reads **requirements + design + tasks + test-plan**.
...
| QA only | `agt-quality-assurance` PLAN / VERIFY (AUTOMATE → route to `agt-test-author`) |
...
1. **New feature:** run `/orchestrate` (or ask to “run the SDD pipeline”) with the problem — not only the technical solution.
...
An obvious single-role request (PO only, QA only, PR only) → dispatch that subagent directly; the orchestrator also redirects.
...
| Concern | Where |
|---------|--------|
| **Kit** SemVer | Kit repo `VERSION` / `CHANGELOG.md` → `.claude/KIT_VERSION` after sync ([ADOPTION.md](../docs/ADOPTION.md)) |
| Service **package** release | [release.md](rules/release.md) (semantic-release / Conventional Commits in the service) |
...
| Doc | Purpose |
|-----|---------|
| [README.md](README.md) | Kit index — rules, agents, skills, quality, ops |
