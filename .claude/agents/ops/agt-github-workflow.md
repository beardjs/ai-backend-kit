...
description: >-
  ...
  creation using the repository template. Only on explicit user request; never
  invoke proactively.
...
- [github-workflow skill](../../skills/github-workflow/SKILL.md) — commit types, order, PR mapping
- [reference.md](../../skills/github-workflow/reference.md) — org defaults (base branch, attribution policy)
- [.github/PULL_REQUEST_TEMPLATE.md](../../../.github/PULL_REQUEST_TEMPLATE.md) — PR body structure
- [release rule](../../rules/release.md) — Conventional Commits for semantic-release
- [tests rule](../../rules/tests.md) — Jest `when` / `should` naming (mandatory for test files)
...
- **Never** add AI/IDE attribution to commits or PRs — forbidden: `Made with Cursor`, `Generated with Claude Code`, `Co-authored-by: Claude`, emoji “Generated with …” trailers, or any similar footer. Strip them if a tool appends them. See [git-no-ai-attribution.md](../../rules/git-no-ai-attribution.md).
...
**Mandatory Jest naming** ([tests.md](../../rules/tests.md)):
...
2. Fill every section of [PULL_REQUEST_TEMPLATE.md](../../../.github/PULL_REQUEST_TEMPLATE.md):
   ...
   - **What has changed?** — check boxes from commit types (`feat` → New feature, `fix` → Bug fix, `service.yaml` → alters contract)
...
**Scope examples:** `user`, `contracts`, `claude`, `domain`
...
chore(claude): add github workflow agent and skill
```
...
- [ ] No AI/IDE attribution in any commit message or PR body
